...
from django_govbr_auth.fetchers import BaseUserInfoFetcher, DefaultEndpointsUserInfoFetcher, run_user_info_fetcher_chain
from django_govbr_auth.utils import get_govbr_settings
...
def test_default_endpoints_user_info_fetcher():
    ...
    mock_client.get_endpoint_data.side_effect = lambda token, path: {
        "/userinfo": {"sub": "12345678901", "name": "Maria Silva"},
    }.get(path, {})
    ...
    fetcher = DefaultEndpointsUserInfoFetcher(govbr_settings=cfg)
    result = fetcher.fetch(mock_client, "fake-access-token")

    assert result["sub"] == "12345678901"
...
def test_run_user_info_fetcher_chain(settings):
    settings.GOVBR_AUTH = {
        ...
        "USER_INFO_ENDPOINTS": ["/userinfo"],
    }

    mock_client = MagicMock()
    mock_client.get_endpoint_data.return_value = {"sub": "12345678901"}
