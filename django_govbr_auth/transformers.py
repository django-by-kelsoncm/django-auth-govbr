...
def parse_date(value, govbr_info=None, date_format="%Y-%m-%d"):
...
def format_cpf(value, govbr_info=None):
...
def to_upper(value, govbr_info=None):
    """Convert value to uppercase string."""
    return str(value).upper() if value is not None else ""


def to_lower(value, govbr_info=None):
    """Convert value to lowercase string."""
    return str(value).lower() if value is not None else ""


def to_bool(value, govbr_info=None):
...
def fetch_image_file(value, govbr_info=None, timeout=10, filename=None):
    ...
    try:
        ...
        if not filename:
            ...
            filename = os.path.basename(path) or "govbr_photo.jpg"
