...
class BaseUserMapper:
    ...
    def __init__(self, govbr_settings=None):
        self.govbr_settings = govbr_settings or {}
...
class DefaultAttrMapUserMapper(BaseUserMapper):
    """Default User Info Mapper link applying GOVBR_AUTH['USER_ATTR_MAP']."""

    def map_attributes(self, user_info, attrs=None):
        if attrs is None:
            attrs = {}

        attr_map = self.govbr_settings.get("user_attr_map", {})
...
BaseGovBrUserMapper = BaseUserMapper
DefaultGovBrUserMapper = DefaultAttrMapUserMapper


def get_user_info_mappers(cfg=None):
    """Instantiate and return the list of mappers in the Chain of Responsibility."""
    from .utils import get_govbr_settings

    if cfg is None:
        cfg = get_govbr_settings()

    mapper_targets = cfg.get(
        "user_info_mappers", ["django_govbr_auth.mappers.DefaultAttrMapUserMapper"]
    )
    mappers = []
    ...
    for target in mapper_targets:
        ...
        if inspect.isclass(cls):
            mappers.append(cls(govbr_settings=cfg))
        elif callable(cls):
            mappers.append(cls)
